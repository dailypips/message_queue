...
private:
    ...
    uint8_t pad0[MQ_CACHE_LINE_SIZE -
                 sizeof(receive_mutex_) -
                 sizeof(receive_wait_cond_) -
                 sizeof(receive_wait_mutex_) -
                 sizeof(receive_wait_) -
                 sizeof(n_receiver_)
                 ];

    uint8_t pad1[MQ_CACHE_LINE_SIZE];
    Node* volatile first_;
    uint8_t pad2[MQ_CACHE_LINE_SIZE - sizeof(first_)];

    volatile int32_t receiveLock_;
    uint8_t pad3[MQ_CACHE_LINE_SIZE - sizeof(receiveLock_)];

    Node* volatile last_;
    uint8_t pad4[MQ_CACHE_LINE_SIZE - sizeof(last_)];

    volatile int32_t sendLock_;
    uint8_t pad5[MQ_CACHE_LINE_SIZE - sizeof(sendLock_)];
};
